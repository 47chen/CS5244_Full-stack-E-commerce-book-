...
public class ApiResource {
    ...
    public Category categoryByName(@PathParam("category-name") String categoryName,
        ...
        try {
            ...
            if (result == null) {
                throw new ApiException(String.format("No such category id: %d", categoryName));
            }
            return result;
        } catch (Exception e) {
            throw new ApiException(String.format("Category lookup by category-id %d failed", categoryName), e);
        }
    }
    ...
    //TODO: Before project 9, we are getting source from DB in client, now we are going to give info. to server,
    //So we're posting thing right here, not get

    @POST
    @Path("orders") //this where you set your order restAPI
    ...
    public OrderDetails placeOrder(OrderForm orderForm) {
        ...
        try {
            ...
            // placeOrder successfully will give you primary key of the order (orderId)
